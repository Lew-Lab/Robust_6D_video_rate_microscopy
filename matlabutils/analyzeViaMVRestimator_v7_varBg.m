% this is for time-variant background - the code is for background
% estimated every 255 frames, update the code if necessary

function locData = analyzeViaMVRestimator_v7_varBg(rawData,b,Bstruct,systemPar,RoSEreg,fName,varargin)
...
for k = 1:bNum
    for chNum = 1:8
        if size(b,1) == PSFsz
            b_ch{chNum,k} = rot90(b(:,PSFsz*(chNum-1)+(1:PSFsz),k),5-rem(chNum,4));
        elseif numel(b) == 1
            b_ch{chNum,k} = b;
        end
    end
...
for groupNum = 1:ceil(totFrame/500)
    ...
    parfor frameNum = frameList
        ...
        for chNum = 1:8
            ...
            [~,~,locDataTmp1] = RoSE2D(obj, SMLM_img, b_ch{chNum,ceil((frameNum+10)/255)}, FPSF, 'regVal', RoSEreg); % background every 255 frames
        ...
        if ~isempty(locDataFrame)
            ...
            for chNum = 1:8
                ...
                if numel(b) ~= 1
                    bCrop = [bCrop,b(6:end-5,(chNum-1)*PSFsz+(6:PSFsz-5),ceil((frameNum+10)/255))]; % background estimated every 255 frames
                else
                    bCrop = b;
                end
            end
