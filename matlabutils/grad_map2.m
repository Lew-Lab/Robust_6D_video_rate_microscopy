...
for i = 1:n_frames
    ...
    if ~isempty(x_localized_xx_t) % if found a local max (source pixel) with high  likelihood
        ...
        for ll = 1:numel(x_localized_xx_t)
            ...
            x_localized = x_localized_xx_t(ll);

            y_localized = y_localized_xx_t(ll);
            ...
            gammaf(indx_lin+1*n_grid_p, i) = br_xx_t(ll) * deltax_xx * 10^-2;
            gammaf(indx_lin+2*n_grid_p, i) = br_xx_t(ll) * deltay_xx * 10^-2;
