function [basis_matrix,mask,BFP_matrix,basis_matrix_dx,basis_matrix_dy] = basis_matrix_pixel_based_v4_2channel_mask(Microscopy,pmask)
...
if isnan(pmask)==1
    ...
    for pmask_channel = 1:2
        ...
        angle_temp = rot90(angle_temp,rot);
        angle_1_sgl_ch = ones(sampling_size, sampling_size)*127;
        angle_temp = im2double(angle_temp,'indexed');
        angleResize = imresize(angle_temp,upsamping);
        ...
        [psf_radius,~] = size(angleResize);
        region = center+[-psf_radius/2:psf_radius/2-1];
        angle_1_sgl_ch(region ,region+3)=angleResize(:,:);
