function [basisgxPol,basisgyPol,basisGxPol,basisGyPol] = sim_BFPgbasis_IPGbasis(Microscopy,pmask,BFPMod)
...
[a,~,~] = size(gxPol);
bfp_radius = 165;
BFPregion = (round((a-1)/2)-(bfp_radius-1)/2):1:(round((a-1)/2)+(bfp_radius-1)/2);
basisgxPol = gxPol(BFPregion,BFPregion,:);
basisgyPol = gyPol(BFPregion,BFPregion,:);
...
switch BFPMod
    case 'xyPol'
        ...
        Gxx = fliplr(flipud(Gxx'));
        Gxy = fliplr(flipud(Gxy'));
        Gxz = fliplr(flipud(Gxz'));
        Gyx = flipud(Gyx);
        Gyy = flipud(Gyy);
        Gyz = flipud(Gyz);
...
% Normalize the Image Plane basis fields
% Normalize by \iint(Gxx^2 + Gxy^2)dxdy = 1
A = 1/sqrt(sum((abs(Gxx).^2 + abs(Gxy).^2),'all'));
Gxx = Gxx.*A; Gxy = Gxy.*A; Gxz = Gxz.*A;
Gyx = Gyx.*A; Gyy = Gyy.*A; Gyz = Gyz.*A;
